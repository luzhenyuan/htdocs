...
function get_html_jg($url){
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER,
        array(
            'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
            'content-type: application/x-www-form-urlencoded',
            'Cookie: PHPSESSID='.$GLOBALS['jg_PHPSESSID'],
          )); 
    // curl_setopt($ch, CURLOPT_ENCODING, "gzip,deflate");
    curl_exec($ch);
    curl_close($ch);
...
function item_level(){ 
    //等级
    foreach(Get_BigNumItem() as $x){
        get_html1("http://192.168.1.242/gmsys_mszc_lzy/web/index.php?actionid=404&t=4&itemID=".$x."&roleID=".$GLOBALS['post']['showID1']."&itemLevel=".$GLOBALS['post']['itemlevel']);
    }
}
function Send_Small(){ 
    //发信使和战车
...
function One_key_strongest_jg(){
    ...
    foreach($money as $x){
        if ($x === '1001' or $x === '1002') {
            $GLOBALS['post']['itemNum'] = 100000000;//默认数量
            $GLOBALS['post']['itemId'] = $x;
            get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
        }else{
            $GLOBALS['post']['itemNum'] = 10000;//默认数量
            $GLOBALS['post']['itemId'] = $x;
            get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
    ...
    //发英雄
    foreach(Get_BigNumItem() as $z){
        $GLOBALS['post']['itemNum'] = 90000;//默认数量
        $GLOBALS['post']['itemId'] = $z;
        get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
    }
    //发信使和战车
    $GLOBALS['post']['itemNum'] = 500;//默认数量
    foreach(Get_SmallNumItem() as $y){
        $GLOBALS['post']['itemId'] = $y;
        get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
    }

    //发头像框和皮肤
    $GLOBALS['post']['itemNum'] = 1;//默认数量
    foreach(Get_other() as $m){
        $GLOBALS['post']['itemId'] = $m;
        get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
    }
...
case '修改':
    ...
    break;
case '修改金戈':
    if($GLOBALS['post']['itemId'] === ""){
        // print_r($GLOBALS['post']);
        get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=311');
    }elseif($GLOBALS['post']['roleScore']  === ""){
        // print_r($GLOBALS['post']);
        get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
    }else{
        get_html_jg('https://mszctest.gameserver.iplay11g.com/gmsys/web/index.php?actionid=316');
